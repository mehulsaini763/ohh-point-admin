...
const VendorsCard3 = ({ profile, vendorId }) => {
  ...
  const getCampaignId = (campaignId) => {
    const foundCampaign = campaignData.find(
      (campaign) => campaign.cid === campaignId
    );
    return foundCampaign ? foundCampaign.campaignId : null;
  };

  return (
    <div className="bg-white rounded-lg p-6 shadow-md min-w-1/2">
      <h2 className="text-sm font-semibold mb-4 text-gray-700">
        List of Campaigns Participated
      </h2>
      {loading ? (
        <p>Loading...</p>
      ) : (
          ...
          <tbody>
            {currentData?.map((campaign, index) => {
              ...
              return (
                <tr
                  key={index}
                  className="hover:bg-gray-100 text-oohpoint-primary-2 text-[12px] font-normal"
                >
                  <td className="px-4 py-1">{campaign.campaignId}</td>
                  <td className="px-4 py-1">
                    {cid ? (
                      <QRCodeGenerator
                        value={`https://user-ooh-point.vercel.app/campaign/${cid}-${vendorId}`}
                      />
                    ) : (
                      <p>No QR Code</p>
                    )}
