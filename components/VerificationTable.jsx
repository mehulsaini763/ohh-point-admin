...
const PaymentTable = ({ data, handleShow }) => {
  ...
  const assignCampaign = async () => {
    setLoading(true);
    if (!campaignId || !vendorId || !pricePerScan || !firstPrize || !secondPrize || !thirdPrize || !fourthPrize || !fifthPrize || !sixthPrize || !firstPercent || !secondPercent || !thirdPercent || !fourthPercent || !fifthPercent || !sixthPercent) {
      setLoading(false);
          ...
          >
            ...
            <div className="flex-1 text-center flex md:gap-3 justify-center max-lg:flex-col gap-1">
              <button onClick={() => {setAssignModal(true)
                ...
                setVendor(person)
              }} className="flex-1 text-[10px] bg-oohpoint-grey-300 hover:bg-oohpoint-grey-400 transition-all font-thin text-oohpoint-grey-100 rounded-md p-[3px]">
                Assign 
              </button>
